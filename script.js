...
function populateBrandFilter() {
    refs.filterBrand.innerHTML = '<option value="">All Brands</option>' + [...new Set(state.products.map(p => p.brand))].sort().map(brand => `<option value="${brand}">${brand}</option>`).join('');
}
    ...
    if (!items.length) {
        refs.sidebarBody.innerHTML = `
            <div class="empty-state">
                <div class="icon">${isCart ? 'Cart' : 'Wishlist'}</div>
                <p>${isCart ? 'Your cart is empty.' : 'No watches in wishlist yet.'}</p>
                <p style="font-size:0.7rem;margin-top:6px;">${isCart ? 'Browse the collection and add your favorites.' : 'Tap the heart icon on any watch to save it.'}</p>
            </div>
        `;
    ...
    refs.sidebarBody.innerHTML = items.map(item => `
        <div class="sidebar-item">
            ...
            ${isCart ? `
                ...
                <button class="sidebar-item-remove" onclick="removeFromCart('${item.id}')">Remove</button>
            ` : `
                <button class="btn-sm gold" onclick="addToCart('${item.id}'); renderSidebar();">Add</button>
                <button class="sidebar-item-remove" onclick="toggleWishlist('${item.id}'); renderSidebar();">Remove</button>
            `}
        </div>
    `).join('');
...
function quickEnquiry(productId) {
    const product = state.products.find(item => item.id === productId);
    if (!product) return;
    const message = `Hi Luxury Watch Co., I am interested in ${product.brand} ${product.name} (${formatPrice(product.price)}). Please share availability and pricing.`;
    window.open(`https://wa.me/918733030195?text=${encodeURIComponent(message)}`, '_blank');
}
...
function initMap() {
    ...
    if (!mapEl) return;
    const liveMap = L.map(mapEl).setView([23.0787, 72.6673], 15);
